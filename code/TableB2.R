...
out <- foreach(i =  1:5000, .packages=c("games2", "brglm", "detectseparation", "Formula"),
               .combine=rbind, .errorhandling = "remove")%dorng%{
                 ...
                 c(total.lp0, total.lp1,total.lp2)
                 ...
                 if(any(class(NC)=="try-error" )|| NC$convergence$code != 0||NC$localID==FALSE || any(abs(NC$coef) > 1000)){
                   NC <- NC.se <- coverage.NC <- power.NC <- rep(NA,4) 
                   pB.NC <- rep(NA,2)
                   pB.NC.0 <- pB.NC.1 <- NA
                 }else{
...
outputB2 <- list(rmse=c(rmseSBI.nc, rmseSBI.wc, rmseFML.nc, rmseFML.wc,rmseFML.wc2,rmseFML.wc3),
...
outputB2$est[seq(1,k*6,by=4)] <- outputB2$est[seq(1,k*6,by=4)]*-1
outputB2$coverage <- c(colMeans(out[,Index$coverage[1,]], na.rm=T),
...
outputB2$power <-  c(colMeans(out[,Index$power[1,]], na.rm=T),
...
outputB2$SE <-  c(colMeans(out[,Index$standardErrors[1,]], na.rm=T),
...
tableB2 <- matrix(t(cbind(matrix(outputB2$est,nrow=6, byrow=T),
                         matrix(outputB2$sd, nrow=6, byrow=T),
                         matrix(outputB2$SE, nrow=6, byrow=T),
                         matrix(outputB2$power, nrow=6, byrow=T),
                         matrix(outputB2$coverage, nrow=6, byrow=T)
)), 
ncol=4, byrow=T)
tableB2 <- round(tableB2,2)
tableB2 <- as.data.frame(cbind(tableB2, c(rbind(outputB2$rmse, matrix(NA,4,6)))))
tableB2 <- rbind(tableB2, c(b0,NA), c(sdtrueSBI, NA), c(sdtrueFIML, NA))
colnames(tableB2) <- c("$\\alpha_0$", "$\\alpha_1$", "$\\beta_0$", "$\\beta_1$", "RMSE")
tableB2 <- cbind.data.frame(data.frame(Estimator=c("Ordinary SBI", rep(NA,4),
                                                  "BR-SBI", rep(NA,4),
...
cat("Table B2\n", file="../tables_and_figures/TableB2.md")
cat(kable(tableB2, digits=2), 
    sep="\n",
    file = "../tables_and_figures/TableB2.md",
    append = TRUE)
