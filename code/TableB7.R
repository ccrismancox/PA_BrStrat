...
out <- foreach(i =  1:5000, .packages=c("games2", "brglm", "detectseparation"),
               .combine=rbind, .errorhandling = "remove")%dorng%{
                 ...
                 bigX <- mvrnorm(N, mu=rep(0,3), Sigma = diag(3))
                 xB <- bigX[,2]
                 xA <- bigX[,3]
...
TableB7 <- matrix(t(cbind(matrix(output$est,nrow=6, byrow=T),
...
ncol=6, byrow=T)
TableB7 <- round(TableB7,2)
TableB7 <- as.data.frame(cbind(TableB7, c(rbind(output$rmse, matrix(NA,4,6)))))
TableB7 <- rbind(TableB7, c(b0,NA), c(sdtrueSBI, NA), c(sdtrueFIML, NA))
colnames(TableB7) <- c("$\\alpha_0$", "$\\alpha_1$","$\\alpha_2$",
                       "$\\beta_0$", "$\\beta_1$", "$\\beta_2$", "RMSE")
TableB7 <- cbind.data.frame(data.frame(Estimator=c("Ordinary SBI", rep(NA,4),
                                                   "BR-SBI", rep(NA,4),
...
cat("Table B.7\n",    file = "../tables_and_figures/TableB7.md")
cat(kable(TableB7, digits=2), 
    sep="\n",
    file = "../tables_and_figures/TableB7.md",
    append = TRUE)
