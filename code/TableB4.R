...
out <- out[out[,Index$separation[6]]==1 & (out[,Index$separation[5]]==1 | out[,Index$separation[7]]==1),]
...
TableB4 <- matrix(t(cbind(matrix(output$est,nrow=6, byrow=T),
...
ncol=k, byrow=T)
TableB4 <- round(TableB4,2)
TableB4 <- as.data.frame(cbind(TableB4, c(rbind(output$rmse, matrix(NA,4,6)))))
TableB4 <- rbind(TableB4, c(b0,NA), c(sdtrueSBI, NA), c(sdtrueFIML, NA))
colnames(TableB4) <- c("$\\alpha_0$", "$\\alpha_1$", "$\\beta_0$", "$\\beta_1$", "RMSE")
TableB4 <- cbind.data.frame(data.frame(Estimator=c("Ordinary SBI", rep(NA,4),
                                                   "BR-SBI", rep(NA,4),
...
cat("Table B.4\n",    file = "../tables_and_figures/TableB4.md")
cat(kable(TableB4, digits=2), 
    sep="\n",
    file = "../tables_and_figures/TableB4.md",
    append = TRUE)
