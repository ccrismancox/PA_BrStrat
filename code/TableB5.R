...
out <- foreach(i =  1:5000, .packages=c("games2", "brglm", "detectseparation"),
  ...
  step1WC <- try(brglm(yB~const+xB+Xroot2-1,subset=yA==1, data=mf.sbi, family=binomial(link="probit"), pl=TRUE))
  
  if(class(step1WC)[1]=="try-error"){ 
    step1WC <- step2WC <- list(coef=rep(NA,3))
  }else{
    p4hat <- pnorm(with(mf.sbi, cbind(const, xB, Xroot2)) %*% step1WC$coef)
    mf.sbi$Zhat1 <- xA*p4hat/sqrt(2)
    mf.sbi$Zhat2 <- X*p4hat/sqrt(2)
    mf.sbi$Ztrue1 <- xA*p4/sqrt(2)
    mf.sbi$Ztrue2 <- X*p4/sqrt(2)
    step2WC <-  try(brglm(yA~const + Zhat1+Zhat2-1, data=mf.sbi, family=binomial(link="probit"), pl=TRUE))
    step2WC_fix <- try(brglm(yA~const+Ztrue1+Ztrue2-1,data=mf.sbi, family=binomial(link="probit"), pl=TRUE))
    ...
    if(class(step2WC)[1]=="try-error"){ 
      ...
      SBIWC.se <- standardErrors.sbi(yA + yB ~ 1 | 0 | xA+X-1 | xB+X,
                                     mf = data.frame(yA,yB,xA,xB,X),
    ...
    if(any(class(step2WC_fix)=="try-error")){
      step2WC_fix <- list(coef=rep(NA,3))
      SBIWC_fixed <- c(step2WC_fix$coef,step1WC$coef)
      
      SBIWC_fixed.se <- rep(NA, 6)
      coverage.WC.fixed.SBI <- rep(NA,6)
      power.WC.fixed.SBI <- rep(NA,6)
      
    }else{
      SBIWC_fixed <- c(step2WC_fix$coef,step1WC$coef)
      
      SBIWC_fixed.se <- standardErrors.sbi(yA + yB ~ 1 | 0 | xA+X-1 | xB+X, mf = data.frame(yA,yB,xA,xB,X),
  ...
  step1NC <- try(glm(yB~const+xB+Xroot2-1,subset=yA==1, data=mf.sbi, family=binomial(link="probit")))
  if(any(class(step1NC)=="try-error")|| step1NC$converged==FALSE||anyNA(step1NC$coef)){
    step1NC <- step2NC <- list(coef=rep(NA,3))
  }else{
    p4hat <- pnorm(with(mf.sbi, cbind(const, xB, Xroot2)) %*% step1NC$coef)
    mf.sbi$Zhat1 <- xA*p4hat/sqrt(2)
    mf.sbi$Zhat2 <- X*p4hat/sqrt(2)
    ...
    step2NC <- try(glm(yA~const+Zhat1+Zhat2-1, data=mf.sbi, family=binomial(link="probit")))
    step2NC_fix <- try(glm(yA~const+Ztrue1+Ztrue2-1,data=mf.sbi, family=binomial(link="probit")))
    if(any(class(step2NC)=="try-error") || step2NC$converged==FALSE){
      ...
      SBINC.se <- standardErrors.sbi(yA + yB ~ 1 | 0 | xA+X-1 | xB+X,
                                     mf = data.frame(yA,yB,xA,xB,X),
    ...
    if(any(class(step2NC_fix)=="try-error") || step2NC_fix$converged==FALSE){
      step2NC_fix <- list(coef=rep(NA,3))
      SBINC_fixed <- c(step2NC_fix$coef,step1NC$coef)
      
      SBINC_fixed.se <- rep(NA, 6)
      coverage.NC.fixed.SBI <- rep(NA,6)
      power.NC.fixed.SBI <- rep(NA,6)
      
    }else{
      SBINC_fixed <- c(step2NC_fix$coef,step1NC$coef)
      
      SBINC_fixed.se <- standardErrors.sbi(yA + yB ~ 1 | 0 | xA+X-1 | xB+X, mf = data.frame(yA,yB,xA,xB),
  ...
  NC <- try(egame12(yA + yB ~ 1 | 0 | xA+X-1 | xB+X, start='zero', link="probit"))
  if(any(class(NC)=="try-error" )|| NC$convergence$code != 0||NC$localID==FALSE){
    ...
    phat <- pnorm(with(mf, cbind(1,xB,X) %*% NC$coef[4:6]/sqrt(2)))
  ...
  WC <- try(egame12(yA + yB ~ 1 | 0 | xA+X-1 | xB+X, start='zero', penalty="Firth", link="probit"))
  if(any(class(WC)=="try-error" )|| WC$convergence$code != 0||WC$localID==FALSE){
    ...
    phat <- pnorm(with(mf, cbind(1,xB,X) %*% WC$coef[4:6]/sqrt(2)))
  ...
  WC2 <- try(egame12(yA + yB ~ 1 | 0 | xA+X-1 | xB+X, start='zero', penalty="Cauchy", link="probit"))
  if(any(class(WC2)=="try-error" )|| WC2$convergence$code != 0||WC2$localID==FALSE){
    ...
    phat <- pnorm(with(mf, cbind(1,xB,X) %*% WC2$coef[4:6]/sqrt(2)))
  ...
  WC3 <- try(egame12(yA + yB ~ 1 | 0 | xA+X-1 | xB+X, start='zero', penalty="logF", link="probit"))
  if(any(class(WC3)=="try-error" )|| WC3$convergence$code != 0||WC3$localID==FALSE){
    ...
    phat <- pnorm(with(mf, cbind(1,xB,X)%*% WC3$coef[4:6]/sqrt(2)))
...
TableB5 <- matrix(t(cbind(matrix(output$est,nrow=6, byrow=T),
...
ncol=6, byrow=T)
TableB5 <- round(TableB5,2)
TableB5 <- as.data.frame(cbind(TableB5, c(rbind(output$rmse, matrix(NA,4,6)))))
TableB5 <- rbind(TableB5, c(b0,NA), c(sdtrueSBI, NA), c(sdtrueFIML, NA))
colnames(TableB5) <- c("$\\alpha_0$", "$\\alpha_1$","$\\alpha_2$",
                      "$\\beta_0$", "$\\beta_1$", "$\\beta_2$", "RMSE")
TableB5 <- cbind.data.frame(data.frame(Estimator=c("Ordinary SBI", rep(NA,4),
                                                  "BR-SBI", rep(NA,4),
                                      ...
                                      Quantity=c( rep(c("Est","St.Dev.", "St.Err.", "Power", "Coverage"), 6),
                                                  ...
                                                  "St. Err. (FIML)")),
                           TableB5)
cat("Table B.5\n",    file = "../tables_and_figures/TableB5.md")
cat(kable(TableB5, digits=2), 
    sep="\n",
    file = "../tables_and_figures/TableB5.md",
    append = TRUE)
